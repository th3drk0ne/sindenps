#!/usr/bin/env bash
#
# Sinden Anti-Uberlag tuner with Pi4/Pi5 model-specific presets
# - Raspberry Pi OS / Debian-based
# - Safe: backs up files before changes
#
# Usage:
#   sudo bash sinden-anti-uberlag.sh
#
set -euo pipefail
...
TUNE_SINDEN_XML="1"                   # 1=apply XML appSettings tuning if files exist

# Default Sinden config paths (adjust if needed)
SINDEN_XML_PATHS=(
  "/home/sinden/Lightgun/PS2/LightgunMono.exe.config"
  "/home/sinden/Lightgun/PS1/LightgunMono.exe.config"
)

# Generic (fallback) Sinden KV
declare -A SINDEN_KV=(
  [CameraExposure]="30"
  [CameraBrightness]="120"
  [CameraContrast]="100"
  [MatchOnlyWherePointing]="1"
  [ConsoleOutputVerbose]="0"
)
...
xml_set_key_preserve() {
  # Minimal in-place <add key=".." value=".."/> editor for Sinden configs
  local file="$1" key="$2" value="$3"
  local bak; bak="$(backup_file "$file")"
  [ -f "$file" ] || { echo "  - Skipped (missing): $file"; return; }

  if grep -qiE "<add[^>]*\\bkey=[\"']${key}[\"']" "$file"; then
    sed -i -E \
      "s#(<add[^>]*\\bkey=[\"']${key}[\"'][^>]*\\bvalue=)[\"'][^\"']*[\"']#\\1\"${value}\"#I" \
      "$file"
    echo "  - ${file}: updated key='${key}' -> '${value}' (backup: $bak)"
  else
    local indent="  "
    local found_indent
    found_indent="$(grep -Eo '^[[:space:]]*<add\b' "$file" | head -n1 | sed -E 's/<add.*//' || true)"
    [ -n "${found_indent}" ] && indent="${found_indent}"
    awk -v KEY="$key" -v VAL="$value" -v IND="$indent" '
      BEGIN{IGNORECASE=1}
      /<\/appSettings[[:space:]]*>/ && !done {
        print IND "<add key=\"" KEY "\" value=\"" VAL "\" />"
        done=1
      }
      { print }
      END{
    ...
    ' "$file" > "${file}.tmp"
    install -m 664 "${file}.tmp" "$file"
    rm -f "${file}.tmp"
    echo "  - ${file}: inserted key='${key}' value='${value}' (backup: $bak)"
  fi
}
...
# -----------------------------
require_root
echo "== Sinden Anti-Uberlag (Pi model presets) =="
...
# 6) Tune Sinden XML appSettings
if [ "${TUNE_SINDEN_XML}" = "1" ]; then
  for f in "${SINDEN_XML_PATHS[@]}"; do
    if [ -f "$f" ]; then
      echo "Tuning Sinden config: $f"
      for k in "${!SINDEN_KV[@]}"; do
        xml_set_key_preserve "$f" "$k" "${SINDEN_KV[$k]}"
      done
    fi
  done
fi
