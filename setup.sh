...
(
  ...
  chown sinden:sinden Default.config High-Resolution.config Recoil-Arcade-Light.config Recoil-Arcade-Strong.config Recoil-MachineGun.config Recoil-Shotgun.config Recoil-Soft.config
...
for FILE in logo.png ps1.png ps2.png load.png offline.png favicon.ico hb.png pal.png ntsc.png apple-touch-icon.png; do
  case "$FILE" in
...
write_profile_aliases() {
  # cleanup legacy aliases instead of creating new ones
  log "Removing legacy picocom alias file if present"

  if [[ -f "$PROFILE_SNIPPET" ]]; then
    rm -f "$PROFILE_SNIPPET"
    log "Removed $PROFILE_SNIPPET"
  else
    log "No legacy alias file found"
  fi
}
